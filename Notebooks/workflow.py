...
def Larry_simples_inverso(df, tempo_fechar = -5, excluir_seguidos = True, acao = 'all'):
    ...
    #Escolher o tamanho do stop
    df['stop'] = df.apply(lambda x: x['High'] + (x['High'] - x['Low'])  if x['acao']=='sell' else x['High'] if x['acao'] == 'sell' else 0, axis =1)
    #df['stop'] = df.apply(lambda x: x['High'] + 5  if x['acao']=='call' else x['High'] if x['acao'] == 'sell' else 0, axis =1)
